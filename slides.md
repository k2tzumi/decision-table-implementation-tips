...
決定ルールはジェネレーター関数で定義します。イテレータとして条件の組み合わせを生成し、yield と yield from で柔軟に手続き的にルールを宣言できます。  


時系列で条件を変化させます。制度変更や期間限定ルールに対応し、日付による条件の切り替えを明示的に表現できます。
ルールの実態クラスのオブジェクトは別途生成します。規約ベースでルールを生の配列で記述する方が、見やすくメンテナンスしやすいです。必要なケースだけを遅延評価でオブジェクト生成させます。大半のオブジェクトが参照、評価、されないのでジェネレーターと相性が良いです。
実際の実装例では、時系列で条件を変化させたり、部分的に繰り返しルール適用したりする場合に、ジェネレーター関数を使用することで効率的に実装できます。
ルールの実態クラスのオブジェクト生成は外側で行い、ルール自体は遅延評価します。オブジェクト生成は外側で行い、ルール自体は遅延評価することで、効率的な処理が可能です。
-->
